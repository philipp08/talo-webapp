...
import {
  ...
  X, Image as ImageIcon, MapPin, Calendar, Info
} from "lucide-react";
    ...
    <div className="flex flex-col w-full gap-4 px-4 pt-3 pb-24">
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          <div key={i} className="rounded-[18px] p-3 bg-[#121212] border border-[#242424] shadow-sm">
            ...
            <p className="text-[8px] font-semibold tracking-wider text-[#8A8A8A] mt-0.5 uppercase">{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between px-0.5">
          <span className="text-[8px] font-black tracking-[0.2em] text-[#8A8A8A] uppercase">FORTSCHRITT</span>
          <span className="text-[9px] text-[#8A8A8A]">95/120 on track</span>
        </div>
        <div className="rounded-[18px] bg-[#121212] border border-[#242424] overflow-hidden">
          {[
            ...
            <div key={i} className="px-3 py-3 border-b border-[#242424] last:border-0 hover:bg-white/[0.02] transition-colors cursor-pointer" onClick={() => onNavigate("members")}>
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-2">
                  <div className="w-[20px] h-[20px] rounded-full bg-[#1A1A1A] border border-[#242424] flex items-center justify-center text-[7px] font-black text-white">{m.name.substring(0,2).toUpperCase()}</div>
                  <span className="text-[11px] font-medium text-white">{m.name}</span>
                </div>
                <span className="text-[11px] font-bold text-white">{m.pts}<span className="text-[#8A8A8A] font-medium">/{m.target}</span></span>
              </div>
              <div className="h-[4px] rounded-full bg-[#1A1A1A] overflow-hidden">
...
function ApprovalScreen() {
  return (
    <div className="flex flex-col w-full gap-3 px-4 pt-3 pb-24">
      <p className="text-[17px] font-bold text-white mb-2">Genehmigung</p>
      {[
        ...
        <div key={i} className="rounded-[18px] bg-[#121212] border border-[#242424] p-3 flex flex-col gap-2.5">
          <div className="flex items-center gap-3">
            <span className="w-[24px] h-[24px] rounded-md flex items-center justify-center text-[9px] font-black text-white" style={{ background: e.catColor }}>{e.cat}</span>
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-bold text-white truncate">{e.name}</p>
              <p className="text-[10px] text-[#8A8A8A] font-medium">{e.user}</p>
            </div>
            <div className="text-right">
              <p className="text-[12px] font-bold text-white font-mono">{e.pts} Pkt.</p>
              <p className="text-[9px] text-[#383838] font-bold uppercase tracking-wider">{e.date}</p>
            </div>
          </div>
          {e.note && (
            <div className="flex items-start gap-1.5 px-1 py-1 rounded-lg bg-white/[0.02]">
              <Info className="w-3 h-3 text-[#383838] mt-0.5" />
              <span className="text-[10px] text-[#8A8A8A]">{e.note}</span>
            </div>
          )}
          <div className="flex items-center gap-2 mt-1">
            <button className="flex-1 py-1.5 rounded-xl bg-white/[0.05] border border-white/[0.05] text-[10px] font-bold text-[#8A8A8A]">ABBLEHNEN</button>
            <button className="flex-1 py-1.5 rounded-xl bg-white text-black text-[10px] font-black">GENEHMIGEN</button>
          </div>
        </div>
      ))}
    </div>
...
function MembersScreen() {
  return (
    <div className="flex flex-col w-full gap-3 px-4 pt-3 pb-24">
      <p className="text-[17px] font-bold text-white mb-2">Mitglieder</p>
      <div className="space-y-2">
        {[
          ...
          <div key={i} className="flex items-center gap-3 p-3 rounded-[18px] bg-[#121212] border border-[#242424]">
            <div className="w-[32px] h-[32px] rounded-full bg-white/[0.05] border border-white/[0.05] flex items-center justify-center text-[10px] font-black text-white">
            ...
            <span className="text-[12px] font-bold font-mono" style={{ color: m.color }}>{m.pts}</span>
          </div>
        ))}
      </div>
    </div>
...
function ActivitiesScreen() {
  return (
    <div className="flex flex-col w-full gap-3 px-4 pt-3 pb-24">
      <p className="text-[17px] font-bold text-white mb-2">Tätigkeiten</p>
      <div className="space-y-2">
        {[
          ...
          <div key={i} className="flex items-center gap-3 p-3 rounded-[18px] bg-[#121212] border border-[#242424]">
            <span className="w-[24px] h-[24px] rounded-md flex items-center justify-center text-[9px] font-black text-white" style={{ background: a.catColor }}>{a.cat}</span>
            <div className="flex-1 min-w-0">
               <p className="text-[12px] font-bold text-white">{a.name}</p>
               <p className="text-[9px] text-[#34C759] font-bold uppercase tracking-wider">{a.pts} Pkt.</p>
            </div>
...
function MoreScreen() {
  return (
    <div className="flex flex-col w-full gap-4 px-4 pt-3 pb-24">
      <p className="text-[17px] font-bold text-white mb-2">Mehr</p>
      <div className="grid grid-cols-1 gap-2">
         {[
           ...
           <div key={i} className="flex items-center gap-3 p-3 rounded-[18px] bg-[#121212] border border-[#242424]">
             <div className="w-[36px] h-[36px] rounded-xl bg-white/[0.03] flex items-center justify-center">{item.icon}</div>
       ...
       <div className="space-y-4">
          <div className="space-y-1.5">
             ...
             <div className="p-3 rounded-2xl bg-[#121212] border border-[#242424] flex items-center justify-between">
                <span className="text-[12px] text-white">Anfänger Training leiten</span>
                <ChevronRight size={14} className="text-[#383838]" />
             </div>
          </div>
          
          <div className="space-y-1.5">
             <label className="text-[9px] font-bold text-[#8A8A8A] uppercase tracking-widest px-1">Datum & Zeit</label>
             <div className="flex gap-2">
                <div className="flex-1 p-3 rounded-2xl bg-[#121212] border border-[#242424] flex items-center gap-2">
                   <Calendar size={12} className="text-[#34C759]" />
                   <span className="text-[12px] text-white">Heute</span>
                </div>
                <div className="flex-1 p-3 rounded-2xl bg-[#121212] border border-[#242424] flex items-center gap-2">
                   <Clock size={12} className="text-[#34C759]" />
                   <span className="text-[12px] text-white">16:00 Uhr</span>
                </div>
             </div>
          </div>
          ...
          <div className="space-y-1.5">
             ...
             <div className="w-full aspect-[4/3] rounded-2xl border-2 border-dashed border-[#242424] bg-[#0c0c0c] flex flex-col items-center justify-center gap-2">
                ...
                <span className="text-[11px] text-[#383838]">Protokoll o. Foto hochladen</span>
             </div>
          </div>
          
          <button className="w-full py-3.5 mt-4 rounded-2xl bg-white text-black font-black text-[13px] shadow-xl shadow-white/5 active:scale-95 transition-transform">
             EINTRAG ABSENDEN
          </button>
       </div>
    </div>
...
export default function PhoneDemo({ initialScreen = "dashboard" }: PhoneDemoProps) {
  ...
  // Correcting for external navigational sets
  useEffect(() => {
    ...
    <div className="flex flex-col items-center w-full max-w-md mx-auto relative z-10 py-4">
      ...
      >
        <div className="w-full h-full rounded-[40px] overflow-hidden relative flex flex-col" style={{ background: "#080808" }}>
          ...
          {/* FAB: Eintragen (Pill inside the content area) - only when screen is NOT entry */}
          {activeScreen !== "entry" && (
             ...
             {tabs.map((tab) => {
                ...
                return (
                   <button
                     ...
                     className={`relative flex flex-col items-center justify-center w-[48px] h-[42px] rounded-[18px] transition-all duration-200 ${
                        isActive ? "bg-white/10 text-white" : "text-[#8A8A8A] border-transparent"
                     }`}
                   >
                     <Icon size={18} strokeWidth={isActive ? 2.5 : 2} />
                     <span className={`text-[7px] font-bold uppercase tracking-wider mt-0.5 ${isActive ? "opacity-100" : "opacity-0"}`}>
                        {tab.label}
                     </span>
                   </button>
                );
          ...
          {/* Home indicator */}
          <div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 w-[80px] h-[4px] rounded-full bg-white/20 z-50 pointer-events-none" />
        </div>
      </div>

      {/* Caption cleaner */}
      <div className="mt-4 text-center">
         <p className="text-[12px] font-bold tracking-widest text-[#8A8A8A] uppercase opacity-40">Interactive Demo (v2.0)</p>
      </div>
    </div>
  );
}
