...
const GlowButton = ({ label, loading, onClick }: { label: string; loading: boolean; onClick: () => void }) => {
  return (
    <motion.button
      whileTap={{ scale: 0.96 }}
      disabled={loading}
    ...
    <div className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-white dark:bg-[#080808]">
      ...
      <div className="z-10 w-full max-w-[400px] px-6">
        ...
        >
          ...
          {error && <p className="text-sm font-poppins text-red-500 text-center mt-2">{error}</p>}

          <div className="mt-4">
            <GlowButton label={mode === "login" ? "Anmelden" : "Registrieren"} loading={loading} onClick={handleSubmit} />
          </div>
        </motion.div>
      </div>
    </div>
