...
const recentIssues = [
  { tag: "Produkt", title: "Wie Talo bauen – und was wir dabei gelernt haben", date: "März 2026" },
  { tag: "Community", title: "5 Vereine, die ihr Ehrenamt neu gedacht haben", date: "Feb. 2026" },
  { tag: "Feature", title: "Workflows: Automatisierung für jeden Verein", date: "Jan. 2026" },
];
  ...
  const SuccessState = (
    <div className="flex flex-col items-center gap-4">
      ...
      <div>
        ...
        <p className="text-gray-500 dark:text-[#8A8A8A]">Check deinen Posteingang – wir haben dir eine Bestätigungsmail geschickt.</p>
      </div>
    </div>
  );
    ...
    <main className="relative min-h-screen bg-white dark:bg-[#080808]">
          ...
          <ScrollReveal direction="up" delay={0.45}>
            {submitted ? SuccessState : (
              <div className="flex flex-col items-center gap-3">
                <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row items-center gap-3 max-w-md w-full mx-auto">
                  <input
                    type="email"
                    required
                    value={email}
                  ...
                  >
                    {loading ? <span className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <><span>Anmelden</span><ArrowRight className="w-4 h-4" /></>}
                  </button>
                </form>
        ...
        <div className="max-w-3xl mx-auto px-6">
          ...
          <StaggerContainer className="flex flex-col gap-4">
            {recentIssues.map((issue, i) => (
              <StaggerItem key={i}>
                <div className="flex items-center justify-between p-5 rounded-[1.25rem] border border-gray-100 dark:border-white/[0.06] bg-white dark:bg-[#111]">
                  <div className="flex items-center gap-4">
                    <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-gray-100 dark:bg-white/[0.07] text-gray-500 dark:text-gray-400">{issue.tag}</span>
      ...
      <section className="py-16 px-6">
        <div className="max-w-3xl mx-auto">
          <ScrollReveal direction="up">
            <div className="relative overflow-hidden rounded-[2rem] bg-black p-10 md:p-16 text-center">
              <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                <div className="w-[500px] h-[300px] rounded-full bg-violet-500/20 blur-[80px]" />
              </div>
              <div className="relative z-10">
                <h2 className="text-2xl md:text-4xl font-semibold font-logo text-white mb-4">Verpasse nichts.</h2>
                <p className="text-white/60 mb-8 max-w-sm mx-auto">Trag dich jetzt ein und erhalte die nächste Ausgabe direkt in dein Postfach.</p>
                {submitted ? (
