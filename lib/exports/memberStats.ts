...
export const isExemptMember = (
  member: Member,
  memberTypeFactors?: Record<string, number>
): boolean => {
  // If there's a custom factor for this type, they are NOT exempt
  if (memberTypeFactors?.[member.memberType] !== undefined) return false;
  return (
...
export const missingPointsForMember = (
  ...
  if (isExemptMember(member, club?.memberTypeFactors)) return 0;
...
export const memberStatusLabel = (
  ...
  if (isExemptMember(member, club?.memberTypeFactors)) return "Befreit";
